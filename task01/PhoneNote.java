...
public class PhoneNote {
    ...
    public static int Add(int oldAmount, int howMuch, PhoneNumber[][] P, Entry[] en) {
        int newAmount, j = 0, number;
        String name;
        String surname;
        String category;
        String type;
        newAmount = oldAmount + howMuch;
        Scanner scanner = new Scanner(System.in);
    ...
    public static int ChoiceId(int amount) {
        int id;
        Scanner scanner = new Scanner(System.in);
        System.out.println("Вы можете покинуть данную категорию меню нажав 0. /n А чтобы продолжить выберите ID записи :");
        id = scanner.nextInt();
        while (id > amount || id < 0) {
            System.out.println("вы ввели несуществующий ID");
            System.out.println("Вы можете покинуть данную категорию меню нажав 0 /n или выберите ID записи заново сейчас :");
            id = scanner.nextInt();
        }
        return id;
    }
            ...
            while (exit) {
                switch (Menu()) {
                        ...
                        if (choice > 0 && choice <= 4) {
                            ...
                            for (int j = 0; j < max; j++) {
                                ...
                                if (P[id - 1][j].getNumber() != 0) {
                                    System.out.print((j + 1) + "-й номер : ");
                                    str = P[id - 1][j].getType();
                                    System.out.print(str);
                                    y = P[id - 1][j].getNumber();
                            ...
                            scanner.nextLine();
                            System.out.println("Введите тип  нового номера:");
                    ...
                    case 4:
                        ...
                        for (int i = id - 1; i < amount - 1; i++) {
                            en[i] = en[i + 1];
                            en[i].setId(i + 1);
                            P[i] = P[i + 1];

                        }
                        en[amount - 1] = new Entry();
                        P[amount - 1] = new PhoneNumber[max];
                        amount--;
